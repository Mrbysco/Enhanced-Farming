package com.Mrbysco.EnhancedFarming;

public class Reference {
	public static final String MOD_ID = "enhancedfarming";
	public static final String MOD_PREFIX = MOD_ID + ":";
	public static final String MOD_NAME = "Enhanced Farming";
	public static final String VERSION = "1.1.3";
	public static final String ACCEPTED_VERSIONS = "[1.12]";
	
	public static final String CLIENT_PROXY_CLASS = "com.Mrbysco.EnhancedFarming.proxy.ClientProxy";
	public static final String SERVER_PROXY_CLASS = "com.Mrbysco.EnhancedFarming.proxy.ServerProxy";
}
